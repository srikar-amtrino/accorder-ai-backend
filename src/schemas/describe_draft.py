...
class DescribeDraftRequest(BaseModel):
    """Request body for the describe and draft agent."""

    query: str = Field(..., description="User query describing the clause to draft.")
    use_document_context: bool = Field(..., description="Whether to use the provided document context for drafting.")
    textinformation: Optional[List[TextInformation]] = Field(default=None, description="List of document paragraphs available as drafting context.")


class ClauseListEntry(BaseModel):
    """"""

    title: str = Field(..., description="Title of the drafted clause.")
    summary: str = Field(..., description="Brief summary of the clause.")
    drafted_clause: str = Field(..., description="Complete contract-ready clause text.")


class DescribeDraftResponse(BaseModel):
    """Response from the describe-draft agent."""

    clauses: List[ClauseListEntry] = Field(default_factory=list, description="List of generated clause drafts.")
