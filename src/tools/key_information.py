...
def get_key_information_stream(content: str) -> Any:
    """Extract structured key contract details from the given document content, streaming results as they arrive."""

    async def event_stream() -> Any:
        async for chunk in llm_model.generate_stream(prompt=_KEY_INFO_USER, context={"contract_text": content}, system_message=_KEY_INFO_SYSTEM):
...
async def get_key_information_generate(content: str) -> ContractAnalyzerResponse:
